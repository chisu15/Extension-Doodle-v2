callDoodle(
  '.doodle-upcoming',
  'https://google-doodle-v2-v2.vercel.app/api/v1/doodle/upcoming'
);

callDoodle(
  '.all-doodle',
  'https://google-doodle-v2-v2.vercel.app/api/v1/doodle/popular'
);

callDoodle(
  '.game-doodle',
  'https://google-doodle-v2-v2.vercel.app/api/v1/doodle/game'
);
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (userData) {
    ...
    callDoodle(
      '.favorite-doodle',
      `https://google-doodle-v2-v2.vercel.app/api/v1/user/favorite/${userData.id}`
    );
  } else {
    document.querySelector(
      '.favorite-doodle'
    ).innerHTML = `<h1 class="empty">Vui lòng đăng nhập để xem doodle yêu thích của bạn</h1>`;
  }
});
...
loginForm.addEventListener('submit', function (event) {
  ...
  let email = loginForm.querySelector(
    ".hidden-menu__input[type='email']"
  ).value;
  let password = loginForm.querySelector(
    ".hidden-menu__input[type='password']"
  ).value;
    ...
    .then((data) => {
      ...
      if (data) {
        ...
        if (user) {
          callDoodle(
            '.favorite-doodle',
            `https://google-doodle-v2-v2.vercel.app/api/v1/user/favorite/${user.id}`
          );
        } else {
...
chrome.storage.local.get('onOffExtension', (res) => {
  ...
  const toggleBtn = document.getElementById(
    'hidden-menu__onOffExtension__toggleButton'
  );
  const toggleContainer = document.querySelector(
    '.hidden-menu__onOffExtension__toggle-default-change'
  );
  const onLabel = document.querySelector(
    '.hidden-menu__onOffExtension__on-label'
  );
  const offLabel = document.querySelector(
    '.hidden-menu__onOffExtension__off-label'
  );
...
document
  .getElementById('btn-random-change')
...
formUpload.addEventListener('submit', (e) => {
  chrome.storage.local.get('doodleUpload', (res) => {
    if (!bufferDoodleUser) {
      alert('Hãy upload doodle của bạn!');
      return;
    }
...
document
  ...
  .addEventListener('click', selectRandomDoodle);
        ...
        item.addEventListener('click', () => {
          ...
          doodleType.forEach((item) => {
            if (item.image == linkDoodle) {
              ...
              nextLinkDoodle =
                doodleType[(currentIndexDoodle + 1) % doodleType.length].image;
...
document
  .getElementById('form-random-change')
...
document.querySelectorAll(".doodle img").forEach((img) => {
  img.addEventListener("click", function () {
      let game = img.getAttribute("alt"); // Lấy URL game từ `alt`
      console.log("[popup.js] Doodle clicked! gameURL:", game);
      ...
      if (game) {
          chrome.runtime.sendMessage({
              ...
              console.log("[popup.js] Message sent to background.js. Response:", response);
          });
      } else {
          console.log("[popup.js] No game URL found for this doodle.");
      }
  });
});
